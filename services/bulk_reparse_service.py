...
本 ticket 是纯下沉重构：KB 级 ``index_status`` 语义修正（#109）、实测 OCR 计数与
报告持久化（#110）、HTTP 端点（#111）都不在这里，后续 ticket 在此基础上加。
"""
    ...
    *,
    ...
    if total:
        with ThreadPoolExecutor(max_workers=max(1, concurrency)) as pool:
            futures = {
                pool.submit(reparse_one, kb_id, t.doc, timeout_s=timeout_s): t.doc
                for t in runnable
            }
