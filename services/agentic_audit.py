...
## 搜索策略
- search_kb：语义向量搜索，适合搜索概念性要求（如"质保期要求"），能匹配同义词
- search_kb_text：精确关键词搜索，适合搜索具体术语/编号（如"IP65"、"GB/T 12345"），速度更快
...
_TOOLS_SPEC = [
    ...
    },
    {
        ...
        "function": {
            "name": "search_kb_text",
            "description": (
                "在知识库中做精确关键词文本搜索（非语义匹配）。"
                "适合搜索具体的标准编号、参数值、专有名词等精确术语。"
                "速度快，不占 GPU，但无法匹配同义词。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "精确搜索关键词（如 GB/T 12345、IP65、3000m²）",
                    },
                },
                "required": ["query"],
            },
