...
- **Spike 的体积影响（issue #66 verification risk #5）** — `@embedpdf/react-pdf-viewer` 把所有 plugin 打 bundle。复测渠道是 `npm run bundle:report`（= `node scripts/report-bundle-size.mjs`）：读 `frontend/dist/assets/*` 报每个 js/wasm/css 文件的原始 + gzip bytes 与 `frontend/scripts/bundle-size-baseline.json`（commit `a0b6a31` 之前的状态）的 delta。当前 spike 提交测得：`dist/assets/index-*.js` 从 `792.63 kB` → `1,903.90 kB`（gzip `235.04` → `507.86 kB`），**单一 index chunk +1,111 kB raw / +272 kB gzip**。`scripts/bundle-size-current.json` 是最新一次 `npm run build` 的快照。替换 PRD 阶段需做 code-split / 按需加载才能回到 baseline 附近。
- **Spike 的 `?highlight=` 判定来源** — `PdfViewerDropin.scanAllPagesByText` 复用 `lib/layoutMatch.blockMatchesHighlight(block, highlight)` 这条新导出的 predicate（与 `matchHighlightToBlocks` 同 T1+P2 语义，但只返回 boolean，便于"知道哪些 block 命中"而无需画布像素 rect 输出）。这样 spike 路径与生产 `matchHighlightToBlocks` 走相同的 N3+includes+LCS 阈值（OCR 字符错场景一致），不引入语义分叉。新增 predicate 在 `frontend/src/lib/layoutMatch.test.ts` 有 9 条覆盖（T1 includes / 双向 includes / 加空格标点 / 全角 NFKC / 长串 OCR LCS 兜底 / 短串 LCS 失阈 / 字符散落不应误命中 / 空 highlight / 完全无关）。
